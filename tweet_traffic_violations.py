...
password_str  = 'SafeStreetsNow2018!' if username == 'safestreets' else ''
is_production = True if username == 'safestreets' else False

# Create a engine for connecting to MySQL
e = create_engine('mysql+pymysql://root:' + password_str + '@localhost/traffic_violations?charset=utf8')

# Create a logger
logger = logging.getLogger('hows_my_driving')
auth = tweepy.OAuthHandler('UzW1c1Gy4FmktSkfXF1dgfupr', '17Uf3vrk44m5fjTUEBPx8sPltX45OfZZtApWhWqt139O0GBgkV')
auth.set_access_token('976593574732222465-YKv9y3mT9Vhu7Ufm7xkfk6Z2T3By3K9', 'B9dNrNT5io8GrB18Roy4eUSqDoR9YvJoErHKfT8aw3sGl')

api = tweepy.API(auth, retry_count=3, retry_delay=5, retry_errors=set([403, 500, 503]))
...
def determine_state(state_input):
    state_abbr_regex   = r'^(AL|AK|AS|AZ|AR|CA|CO|CT|DE|DC|FM|FL|GA|GU|HI|ID|IL|IN|IA|KS|KY|LA|ME|MH|MD|MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|MP|OH|OK|OR|PW|PA|PR|RI|SC|SD|TN|TX|UT|VT|VI|VA|WA|WV|WI|WY)$'
...
def find_campaign_hashtags(string_parts):
...
def form_response_parts(query_result, username):
    ...
    logger.debug("total_violations: %s", total_violations)
    ...
    for violation in query_result['violations']:
        ...
        if (potential_response_length <= MAX_TWITTER_STATUS_LENGTH):
            ...
            logger.debug("length: %s", len(violations_string))
            logger.debug("string: %s", violations_string)
            ...
            logger.debug("violations_string: %s", violations_string)
            logger.debug("length: %s", len(violations_string))
            logger.debug("string: %s", violations_string)
...
def handle_short_response(message_id, message_type, response_message, username):
    if message_type == 'direct_message':
        is_production and api.send_direct_message(screen_name = username, text = response_message)
    else:
        is_production and api.update_status(response_message, in_reply_to_status_id = message_id)
...
def perform_queries(args):
    ...
    logger.debug('Performing lookup for plate.')
    ...
    logger.debug('Listing args... plate: %s, state: %s, message_id: %s, created_at: %s', plate, state, str(message_id), str(created_at))
    ...
    # log response
    logger.debug('violations raw: %s', opacv_response)
    logger.debug('Open Parking and Camera Violations data: %s', opacv_data)
    ...
    for endpoint in fy_endpoints:
        ...
        logger.debug('fy_response: %s', data)
    ...
    logger.debug('violations sorted: %s', result)
    ...
    if previous_data:
      ...
      logger.debug('we have previous data: %s', previous_data[0])
    ...
    logger.debug('returned_result: %s', result)
...
def process_message_response(message, response_args):
    ...
    try:
        ...
        query_info['included_campaigns'] = find_campaign_hashtags(string_parts)
...
def recursively_process_direct_messages(response_parts):
    ...
    for part in response_parts:
        if isinstance(part, list):
            return_message.append(recursively_process_direct_messages(part))
        else:
...
def recursively_process_status_updates(response_parts, message_id):

    # Iterate through all response parts
    for part in response_parts:
        # Some may be lists themselves
        if isinstance(part, list):
            message_id = recursively_process_status_updates(part, message_id)
        else:
            new_message = is_production and api.update_status(part, in_reply_to_status_id = message_id)
            message_id  = is_production and new_message.id

            logger.debug("message_id: %s", str(message_id))
...
class MyStreamListener (tweepy.StreamListener):
    ...
    def on_data(self, data):
        ...
        if 'delete' in data_dict:
            ...
            initiate_reply(status)
            # if self.on_status(status) is False:
            #     return False
        else:
            logging.error("Unknown message type: " + str(data))



    def on_event(self, status):
        logger.debug("on_event: %s", status)
    ...
    def on_direct_message(self, status):
        logger.debug("on_direct_message: %s", status)
