    ...
    def form_plate_lookup_response_parts(self, query_result, username):
        ...
        if query_result.get('camera_streak_data'):
            ...
            if streak_data.get('max_streak') and streak_data['max_streak'] >= 5:
                ...
                streak_string = "Under @bradlander's proposed legislation, this vehicle could have been booted or impounded due to its {} camera violations from {} to {}.\n".format(streak_data['max_streak'], streak_data['min_streak_date'], streak_data['max_streak_date'])

                # add to container
                response_chunks.append(username + ' ' + streak_string)

                # add Simcha Felder string until bill is passed
                simcha_felder_string = "Authorization for NYC's speed safety cameras expires June 30, 2018. Senate Bill 6046A would authorize the city to double the number of life-saving cameras. Call @NYSenatorFelder at 718.253.2015/518.455.2754 and urge him to let the bill out of committee.\n"
